...
def calcula_porcentagem_sorteio(assinante, minutos_assistidos):
    ...
    while i < quantidade:
        if (minutos_assistidos[i] % 60 == 0):
            horas = minutos_assistidos[i] / 60
        else:
            horas = (minutos_assistidos[i] // 60) + 1

        if (assinante[i]):
            a = horas * 2
            horasAssistidas.append(a)
        else:
            a = horas
            horasAssistidas.append(a)
    ...
    for item in horasAssistidas:
        porcentagemTotal += item

    for item in horasAssistidas:
        porcentagem = item * 100 / porcentagemTotal
        porcentagemIndividual.append(round(porcentagem))

    return porcentagemIndividual